...
LOGIT_EMA = 0.70          # 0(안함)~1(느리게 반응). 높을수록 안정, 반응 느려짐
MARGIN_MIN = 0.08         # top1 - top2가 이보다 작으면 스위치 보류
SWITCH_DELTA = 0.10       # 새 라벨 확률이 현재 라벨보다 최소 이만큼 높아야 전환 후보
SWITCH_CONSEC = 4         # 전환 후보가 연속 몇 프레임 유지되어야 전환
MIN_HOLD = {              # 라벨 유지 최소 길이(프레임) : 지나야 전환 허용
    "assault": 32,        # ≈ 1.3s(@24fps)
    "swoon": 60,          # ≈ 2.5s
    "trespass": 40        # ≈ 1.7s
}
MIN_SHOW_CONF = 0.30
...
def run_video(src=0):
    ...
    out_mp4 = os.path.join(OUT_DIR, f"{stem}_sf_maskclass_stable.mp4")
    out_csv = os.path.join(OUT_DIR, f"{stem}_sf_maskclass_stable.csv")
    ...
    writer=None
    buf = deque(maxlen=t_fast); rows=[]; no_person_streak=0; frame_idx=0
    ...
    for frame in read_frames(kind, handle):
        ...
        if len(buf) == t_fast:
            ...
            p = p * mask; s = float(p.sum())
            if s > 1e-8: p /= s
            else: p[:] = 0.0

            # EMA smoothing
            if p_ema is None: p_ema = p.copy()
            else: p_ema = LOGIT_EMA * p_ema + (1.0 - LOGIT_EMA) * p

            # decision
            order = np.argsort(-p_ema)
            k1, k2 = int(order[0]), int(order[1] if len(order)>1 else order[0])
            ...
            if current_label == "":
                ...
                if label1 == current_label:
                    ...
                    # conditions to consider switching
